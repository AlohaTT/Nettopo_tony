...
public class Page_GasTopoAttributes extends WizardPage {
	Text[] txt_args;
	Label[] lbl_args;
	Topology topology;
	...
	public void setVisible(boolean visible){
		if(visible){
			...
			Page_GasTopoType page_gastopoType = (Page_GasTopoType)getWizard().getPage("GasTopoType");
			topology = page_gastopoType.getTopology();
	...
	private boolean checkCoordinates(Coordinate[] coordinates) throws CoordinateNotFoundException{
		...
		for(int i=0; i<coordinates.length;i++){
			if(!coordinates[i].withinRange(NetTopoApp.getApp().getNetwork().getSize())){
				setErrorMessage("Some gas coodinate is out of range");
				return false;
			}
		}
			...
			for(int i=0; i<coordinates.length;i++){
				for(int j=i+1;j<coordinates.length;j++){
					if(coordinates[i].equals(coordinates[j])){
						setErrorMessage("Sorry, some gas' coordinate is duplicate");
						return false;
					}
				}
			}

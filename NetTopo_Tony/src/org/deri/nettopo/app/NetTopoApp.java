...
public class NetTopoApp {
	 ...
	 */
	private Shell sh_main = null;// shell 修改成static
	...
	private void createMenu_main() {
		...
		/* 将新的算法加在文件algorithm.properties里面，即可在菜单中看到，要有相应的类 */
		...
		mi_parallelRun.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					if (!isFinish) {
						addLog("Some thing is worng with slaves.   #101"); // 101号错误
						return;
					}
			...
			public void handleEvent(Event event) {
				...
				case SWT.MouseDown:
					if (event.button == 1) { // left button down
						...
						if (moveNode == null) { // 不是movenode而是有可能是 gas
							int gid_minDis = -1;
					...
					if (event.button == 1) { // 左键
						if (pointInNode) {
							...
							if (wsn.duplicateWithOthers(coordinate)) {
								...
								// by dennis
								// painter.rePaintAllGas();
								currentSelectedNode = moveNode;// 这个功能我看下，如果差不多了就这样先。老师们是想要先出效果
	...
	public void run() {
		...
		sh_main.setLocation(500, 100);// 设置位置
	...
	private void paintNetwork(int index, String pictureName) throws Exception // 重新画图，从slave得到的结果中画图
	{
	...
	private boolean cmd_parallelRun() throws Exception {// 并行
		ReadXML.FILEPATH = CreateTasks.simPath;

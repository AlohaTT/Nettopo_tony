...
public class TPGF_Planarization_GG implements AlgorFunc {
	...
	public void run(){
		// test
		type = GG;
		if(Property.needpaint)
		Planarization(type,true);
		else
		{
			Planarization(type,false);
		}
	}
	...
	public void Planarization(String graphtype, boolean needpaint){
		...
		TPGF_ConnectNeighbors func_connectNeighbors=null;
		try{
			
			if(getAlgorithm()==null)
				 func_connectNeighbors =new TPGF_ConnectNeighbors();
			else
			func_connectNeighbors = (TPGF_ConnectNeighbors)getAlgorithm().getFunctions()[0];
			
		}catch(Exception e)
		{
			func_connectNeighbors =new TPGF_ConnectNeighbors();
		}
			func_connectNeighbors.connectNeighbors(false);//运行获取邻节点集合
		...
		if(sinkNodes==null||sinkNodes.size()==0)
		{
			NetTopoApp.getApp().addLog("There is no sink node.");
			return;
		}

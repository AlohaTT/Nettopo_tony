...
public class TPGF_FindOnePath_forSDN implements AlgorFunc {
	...
	public TPGF_FindOnePath_forSDN(Algorithm algorithm){
		this.algorithm = algorithm;
	}

	public TPGF_FindOnePath_forSDN(){
		this(null);
	}
	...
	public boolean findOnePath(boolean needPainting,SensorNode nodes_source,SinkNode nodes_sink){
...
//			if(nodes_sink.size()>0 && nodes_source.size()>0){
				SensorNode node_source = (SensorNode)nodes_source; // retrieve one source node��diyige
				sink = (SinkNode)nodes_sink;
...
//		}
	...
	public boolean canReachSink(SensorNode node){
		...
		node.setOneHopNeighbors(node);
		List<SensorNode> oneHopNieghbors = node.getOneHopNieghbors();
		Iterator<SensorNode> iterator = oneHopNieghbors.iterator();
		List<Integer> neighborsID = new ArrayList<>();
		while (iterator.hasNext()) {
			SensorNode sensorNode = (SensorNode) iterator.next();
			neighborsID.add(sensorNode.getID());
		}
	...
	/* (non-Javadoc)
	 * @see org.deri.nettopo.algorithm.AlgorFunc#run()
	 */
	@Override
	public void run() {
		// TODO Auto-generated method stub

...
public class TPGF_OptimizeOnePath_forSDN implements AlgorFunc {
	...
	public TPGF_OptimizeOnePath_forSDN(Algorithm algorithm){
...
//		
		...
		//wsn.alterNeighbour(Property.R);
		
		wsn.resetSensorNodesNeighbors();
		
		TPGF_FindOnePath_forSDN func_find = (TPGF_FindOnePath_forSDN)getAlgorithm().getFunctions()[1];
		...
		while(label < path.size()-1){
			...
			for(int i=0;i<neighborsID.size();i++){
				...
				for(int j=label+2; j<path.size(); j++){//Ѱ�����һ��
